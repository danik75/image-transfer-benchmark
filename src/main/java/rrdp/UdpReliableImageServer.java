...
public class UdpReliableImageServer {
    public static void main(String[] args) throws Exception {
        final int port = 7002; // Different port for URDP
        final int bufferSize = 20004; // 20000 + 4 header bytes
        DatagramSocket socket = new DatagramSocket(port);

        System.out.println("URDP Image Server listening on port " + port);
        ...
        while (true) {
            ...
            while (true) {
                ...
                if (chunks.size() == totalChunks) {
                    ...
                    try (FileOutputStream fos = new FileOutputStream("received_image_urdp.jpg")) {
                        for (byte[] part : chunks.values()) {
                            fos.write(part);
                        }
                    }

                    System.out.println("💾 Image saved as received_image_urdp.jpg");
                    break;
                }
            }
        }
